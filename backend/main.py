...
CRISP_API_ID   = os.environ.get("CRISP_API_ID", "")
CRISP_API_KEY  = os.environ.get("CRISP_API_KEY", "")
ADMIN_PASSWORD = os.environ.get("ADMIN_PASSWORD", "changeme")
DB_PATH        = os.environ.get("DB_PATH", os.path.join(os.path.dirname(__file__), "ai-any-camera.db"))
# ─────────────────────────────────────────────────────────────────────────────
...
def auto_add_camera(req: AutoAddRequest, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    db_cam = CameraDB(
        ...
        rtsp_url=result["rtsp_url"], brand=result["brand"],
...
def add_camera(camera: CameraCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    db_cam = CameraDB(
        user_id=current_user.id, name=camera.name,
        rtsp_url=camera.rtsp_url, brand=camera.brand,
...
def register(req: RegisterRequest, db: Session = Depends(get_db)):
    ...
    while db.query(UserDB).filter(UserDB.username == username).first():
        ...
        suffix += 1
    user = UserDB(email=req.email, username=username,
                  hashed_password=hash_password(req.password),
                  verify_token=verify_token, is_verified=0)
    db.add(user)
    try:
        db.commit()
        db.refresh(user)
...
def me(current_user: UserDB = Depends(get_current_user)):
    ...
    if plan == "premium":
        try:
            import sqlite3
            conn = sqlite3.connect(DB_PATH)
            new_subjects = conn.execute(
                "SELECT COUNT(*) FROM subjects WHERE user_id=? AND viewed=0", (current_user.id,)
            ).fetchone()[0]
            conn.close()
        except:
            pass
...
def verify_phone(data: dict, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    conn = sqlite3.connect(DB_PATH)
    row = conn.execute(
        "SELECT phone, code, created_at FROM phone_verify_codes WHERE user_id=? ORDER BY id DESC LIMIT 1",
        (current_user.id,)
    ).fetchone()
    conn.close()
    if not row:
...
def get_subjects(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    conn = sqlite3.connect(DB_PATH)
    rows = conn.execute(
        "SELECT id, name, class, photo_url, first_seen, last_seen, seen_count, camera_id FROM subjects WHERE user_id=? ORDER BY last_seen DESC",
        (current_user.id,)
    ).fetchall()
    conn.close()
    return [{"id": r[0], "name": r[1], "class": r[2], "photo_url": r[3],
             "first_seen": r[4], "last_seen": r[5], "seen_count": r[6], "camera_id": r[7]} for r in rows]

@app.put("/subjects/{subject_id}")
def update_subject(subject_id: int, data: dict, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    import sqlite3
    conn = sqlite3.connect(DB_PATH)
    conn.execute("UPDATE subjects SET name=? WHERE id=? AND user_id=?",
                 (data.get("name"), subject_id, current_user.id))
    conn.commit()
    conn.close()
    return {"ok": True}
...
HUMAN_KEYWORDS = ["talk to human", "real person", "live agent", "human agent",
                   "speak with someone", "talk to agent", "enterprise", "custom quote", "contact sales"]
...
async def crisp_webhook(request: Request):
    ...
    if origin in ("operator", "bot") or not content or not session_id:
        return {"ok": True}
    wants_human = any(kw in content.lower() for kw in HUMAN_KEYWORDS)
    if wants_human:
            ...
            async with httpx.AsyncClient(timeout=30) as client:
                r = await client.post(
                    ...
                    json={"model": "claude-haiku-4-5-20251001", "max_tokens": 300,
                          "system": CRISP_SYSTEM, "messages": [{"role": "user", "content": content}]}
                )
                reply = r.json().get("content", [{}])[0].get("text", "Sorry, try again.")
        except Exception as e:
            print(f"AI error: {e}")
...
def admin_stats(password: str, db: Session = Depends(get_db)):
    ...
    for u in users:
        ...
        users_data.append({"id": u[0], "email": u[1], "username": u[2],
    ...
    return {
        "users": users_data,
        "totals": {"users": len(users_data), "cameras": total_cameras,
                   "events": total_events, "subjects": total_subjects},
...
async def agent_event(request: Request, db: Session = Depends(get_db)):
    ...
    event = EventDB(camera_id=cam.id, camera_name=cam.name,
    ...
    if cam.notify_telegram and frame is not None:
        try:
    ...
    if cam.notify_sms:
        try:
            from sms_service import send_sms
            msg = caption + (f" {photo_url}" if photo_url else "")
...
async def agent_frame(request: Request, db: Session = Depends(get_db)):
    """Receive frame from Pi Zero W agent, run YOLO on server"""
    data = await request.json()
    token = data.get("token")
    if not token:
        raise HTTPException(status_code=401, detail="No token")
    ...
    detect_classes = cam.detect_classes or ["person"]
    detections = detect_frame(frame, detect_classes)
    fh, fw = frame.shape[:2]
    if cam.zone:
        detections = [d for d in detections if in_zone(d, cam.zone, fw, fh)]
    if not detections:
        return {"detected": None}
    ...
    try:
        ...
        print(f"Photo upload error: {e}")
    event = EventDB(camera_id=cam.id, camera_name=cam.name,
                    detected=best["class"], confidence=f"{best['conf']:.2f}",
                    timestamp=now_str, photo_url=photo_url)
    db.add(event)
    db.commit()
    caption = f"🚨 {cam.name}: {best['class']} detected at {now_time}"
    ...
    if cam.notify_sms:
        try:
            ...
            print(f"SMS error: {e}")
    return {"detected": best["class"], "confidence": best["conf"], "event_id": event.id}
...
def _cloudinary_cleanup():
    while True:
        try:
            _time.sleep(3600)
            import cloudinary.api, re, sqlite3
            conn = sqlite3.connect(DB_PATH)
            now = _time.time()
            events = conn.execute("SELECT id, camera_id, photo_url FROM events WHERE photo_url IS NOT NULL").fetchall()
            for eid, cam_id, photo_url in events:
                match = re.search(r'/v(\d+)/', photo_url)
                if not match:
                    continue
                age_days = (now - int(match.group(1))) / 86400
                cam_row = conn.execute("SELECT user_id FROM cameras WHERE id=?", (cam_id,)).fetchone()
                if not cam_row:
                    continue
                user_row = conn.execute("SELECT plan FROM users WHERE id=?", (cam_row[0],)).fetchone()
                plan = user_row[0] if user_row else "free"
                max_days = 1 if plan == "free" else 7 if plan == "pro" else 14
                if age_days > max_days:
                    pub = re.search(r'/aianycamera/(.+)\.jpg', photo_url)
                    if pub:
                        try:
                            cloudinary.api.delete_resources([f"aianycamera/{pub.group(1)}"])
                        except:
                            pass
            conn.close()
        except Exception as e:
...
_threading.Thread(target=_cloudinary_cleanup, daemon=True).start()
